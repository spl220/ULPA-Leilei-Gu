...
$('#language_input').on('focusout', function() {       
        ...
        populate_language_subjects(selected_universities);
      }
});
...
function populate_universities() {
    $.ajax({
        url: '/universities/populate_universities',
        type: 'GET',
        dataType: 'json',
    })
    .done(function(data) {
        ...
        languages = $('#step2 .selectize-input .item');
        $.each(data, function(index, val) {
            language_individual = index;
            language_count = val;
            $.each(languages, function(index, val) {
               if(language_individual == $(this).text()){
                  $('#available_subjects').append('<p>There are <strong>'+language_count+'</strong> subjects available for <strong>'+language_individual+'</strong> in your selected universities</p><br/>');
               }
            }); 
        });
    })
...
function populate_language_subjects(selected_universities) {
    $.ajax({
        url: '/universities/list_potential_languages',
        type: 'GET',
        dataType: 'json',
