...
def university_search(request):
    ...
    for state in state_choices:
        ...
        for university in universities:
            ...
            found_subjects = all_subjects.filter(university__name = university)
            if (len(found_subjects) > 0):
               temp.append((university,len(found_subjects)))
               state_subject_num = state_subject_num + len(found_subjects)

        states.append((state[0], state_subject_num,temp))
...
def list_potential_languages(request):
    university_query = []
    response_languages = []
    all_subjects = Subject.objects.all()
    response_subjects = {}
    ...
    for subject in available_subjects:
        if subject.language.name in response_languages:
            ...
            response_languages += [subject.language.name]
            found_subjects = all_subjects.filter(university__name__in=university_query, language__name = subject.language.name)
            response_subjects[subject.language.name] = [(len(found_subjects))]
...
def list_potential_subjects(request):
    ...
    for subject in available_subjects:
        ...
        if subject.language.name in response_languages:
            ...
            found_subjects = Subject.objects.filter(university__name__in=university_query,language__name = subject.language.name)
            response_subjects += [(subject.language.name,len(found_subjects))]
            response_languages += subject.language.name
    return JsonResponse(response_subjects, safe=False)
